...
- Nếu server trả về `401 Unauthorized` hoặc `403 Forbidden`, `customFetch` sẽ chủ động kích hoạt sự kiện `auth:logout` -> Context bắt được sự kiện này -> Đăng xuất người dùng -> Router đá văng ra `/login`. Đảm bảo hệ thống không bao giờ bị "treo" hay kẹt ở trạng thái "nửa nạc nửa mỡ".
...
1. **Landing Page (`/`)**: Giao diện giới thiệu tính năng, animated background sinh động. Người dùng tìm hiểu nền tảng.
2. **Đăng ký (`/register`)**: Điền form tạo tài khoản -> Đăng ký thành công -> Tự động chuyển qua trang Đăng nhập.
3. **Đăng nhập (`/login`)**: Nhập Email & Mật khẩu -> Bấm Sign In -> Gọi API `/auth/login` -> Trả về Token -> Cập nhật vào `AuthContext` -> Tự động chuyển hướng vào `/dashboard` (hoặc `/onboarding` nếu là lần đầu).
...
- Nhấn hoàn thành -> Cập nhật dữ liệu lên hệ thống -> Chuyển hướng vào trang chính.
...
- Đã sẵn sàng kết nối chặt chẽ với Backend (mọi API endpoints đã được cấu hình trong thư mục `src/services/api.js`).
